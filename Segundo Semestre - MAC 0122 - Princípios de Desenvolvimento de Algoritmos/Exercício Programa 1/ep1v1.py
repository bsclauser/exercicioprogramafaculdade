...
def ProcuraElementoQuadrado (Mat, L, C, p):
	...
	if L >= 6 and L <= 8:
		if C >= 0 and C <= 2:
			...
			quad = QuadradosRetornaEspecifico(Mat, 9)
			cont = 0
			for f in range(3):
				for j in range(3):
					if quad[f][j] == p:
		...
		for y in range (9):
			for j in range(9):
				if Mat[y][j] == 0:
					listasol = []
					for n in range (1,10):
						col = ProcuraElementoColuna(Mat, j, n)
						lin = ProcuraElementoLinha(Mat, y, n)
						qua = ProcuraElementoQuadrado(Mat, y, j, n)
